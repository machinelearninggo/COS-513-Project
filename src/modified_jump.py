...
def get_prob(s):
    l = s[1:] - s[:-1]
    delta = 1e-3
    return (np.count_nonzero(l) + delta) / (len(s) - 1 + 2 * delta)
    

def jump(Y, n_states, jump_penalty=1e-5, tune_decay = 1, tune_loss = 1, initial_states=None,
    ...
    for init in range(n_init):
        prob = get_prob(s)
        x_prob =  (1 - prob) / prob
        theta = np.tanh(x_prob / tune_decay)
        Gamma = (jump_penalty * theta + np.log(x_prob) * (1 - theta)) * (1 - np.eye(n_states))
        ...
        for it in range(max_iter):
            ...
            # Fit state sequence
            s_old = s.copy()
            loss_by_state = tune_loss * cdist(mu, Y, 'euclidean').T**2
